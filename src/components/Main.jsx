...
export default function Main ({data, locationInfo}) {
    ...
    setCurrentTime(new Date().toLocaleString(undefined, { timeZone: locationInfo.tzId }))
    }, 1000)
    ...
    const weeklyCondition = data.daily.map(index => (Object.keys(iconList).find(key => iconList[key].includes(index.weather[0].id)
    )))
 
    const weeklyImageUrls = weeklyCondition.map(condition => iconUrls[condition])


    const hourlyForecast = data.hourly
    .map((hourly,index) => ({
        hour: ((locationInfo.time +index) > 24
    ...
    const next6Days = Array.from({ length: 6 }, (_, i) => daysShort[(dayIndex + 1 + i) % 7])


    const weekCards = data.daily.flatMap((index,i) => ({
  ...
  <div className="main-grid">
    <div>
            ...
            <h3>{data.current.weather[0].main}</h3>
                 ...
                 <section className="weather-detail">
                    ...
                    <div className="weather-detail-part">
                        <img src="/src/assets/icons8-wind-100.png" alt="wind" />
                        <h4>{data.current.wind_speed} kph</h4>
                    </div>
                    <div className="weather-detail-part">
                        <img src="/src/assets/icons8-humidity-96.png" alt="humidity" />
        ...
        <div>
            ...
            <div className="card">{weekCards.map(({ day, highest, lowest,img }, i) => (
             <WeekCard key={i} day={day} highest={highest} lowest={lowest} img={img} />
             ))}</div>
        </section>
        <hr/>
